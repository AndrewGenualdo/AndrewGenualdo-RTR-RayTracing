...
/*
	...
	a3_Scene_Starter-idle-render.c/.cpp
	...
	a3f32 const textAlign, a3f32 const textDepth, a3f32 const textOffsetDelta, a3f32 textOffset)
{
	...
	// pipeline and target
	a3_Scene_Starter_RenderProgramName const render = scene->render;
	a3_Scene_Starter_DisplayProgramName const display = scene->display;
	a3_Scene_Starter_ActiveCameraName const activeCamera = scene->activeCamera;
	a3_Scene_Starter_PipelineName const pipeline = scene->pipeline;
	a3_Scene_Starter_PassName const pass = scene->pass;
	a3_Scene_Starter_TargetName const targetIndex = scene->targetIndex[pass];
	a3_Scene_Starter_TargetName const targetCount = scene->targetCount[pass];
...
void a3starter_render(a3_DemoState const* demoState, a3_Scene_Starter const* scene, a3f64 const dt)
{
	...
	const a3_SceneShaderProgram* renderProgram[starter_pipeline_max][starter_render_max] = {
		{
	...
	const a3_SceneShaderProgram* displayProgram[starter_display_max] = {
		demoState->prog_drawTexture,
	};
	...
	a3_Scene_Starter_PipelineName const pipeline = scene->pipeline;
	a3_Scene_Starter_PassName const pass = scene->pass;
	a3_Scene_Starter_TargetName const targetIndex = scene->targetIndex[pass], targetCount = scene->targetCount[pass];
	a3_Scene_Starter_PassName currentPass;
...
//	a3vec2 pixelSize = a3vec2_one;
	...
	case starter_forward: {
		...
		case starter_renderTexture:
			...
			for (currentSceneObject = scene->obj_plane, endSceneObject = scene->obj_torus,
				...
				++j, ++currentSceneObject)
			{
				// send data and draw
				i = (j * 2 + 11) % hueCount;
				currentDrawable = drawable[currentSceneObject - scene->obj_skybox];
	...
	a3scene_drawModelTexturedColored_invertModel(modelViewProjectionMat.m, viewProjectionMat.m, scene->obj_skybox->modelMat.m, a3mat4_identity.m, currentDemoProgram, demoState->draw_unit_box, demoState->tex_skybox_clouds, a3vec4_one.v);
	...
	// scene overlays
	if (scene->pass >= starter_passScene)
	{
		...
		{
			// scene objects
			for (currentSceneObject = scene->obj_plane, endSceneObject = scene->obj_teapot,
				j = (a3ui32)(currentSceneObject - scene->object_scene);
				currentSceneObject <= endSceneObject;
				++j, ++currentSceneObject)
