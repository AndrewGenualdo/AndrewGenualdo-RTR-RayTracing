...
//-----------------------------------------------------------------------------
				...
				//	-> read number of hierarchy poses (used as parameter)
				//	-> allocate and set pointers
				//  -> read all poses (hposes * nodes)
				//  -> read all channels (nodes)
				//	-> read all orders (nodes)
				a3ui32 hposeCount = 0;
				a3ui32 const poseCountRead = (a3ui32)fread(&hposeCount, sizeof(hposeCount), 1, fp);
				a3ui32 const nodeCount = poseGroup->hierarchy->numNodes;
				a3ui32 const hposeSpace = sizeof(a3_HierarchyPose) * hposeCount;
				a3ui32 const sposeCount = hposeCount * nodeCount, sposeSpace = sizeof(a3_SpatialPose) * sposeCount;
				a3ui32 const channelSpace = sizeof(a3_SpatialPoseChannel) * nodeCount;
				a3ui32 const orderSpace = sizeof(a3_SpatialPoseEulerOrder) * nodeCount;
				a3ui32 const memreq = hposeSpace + sposeSpace + channelSpace + orderSpace;
				...
				// read data
				ret += poseCountRead;
				ret += (a3ui32)fread(poseGroup->pose, sizeof(a3_SpatialPose), sposeCount, fp);
				ret += (a3ui32)fread(poseGroup->channel, sizeof(a3_SpatialPoseChannel), nodeCount, fp);
				ret += (a3ui32)fread(poseGroup->order, sizeof(a3_SpatialPoseEulerOrder), nodeCount, fp);
...
//-----------------------------------------------------------------------------
				...
				//  -> write all poses (hposes * nodes)
				//  -> write all channels (nodes)
				//	-> write all orders (nodes)
				a3ui32 const hposeCount = poseGroup->hposeCount;
				a3ui32 const nodeCount = poseGroup->hierarchy->numNodes;
				a3ui32 const sposeCount = hposeCount * nodeCount;
				ret += (a3ui32)fwrite(&hposeCount, sizeof(hposeCount), 1, fp);
				ret += (a3ui32)fwrite(poseGroup->pose, sizeof(a3_SpatialPose), sposeCount, fp);
				ret += (a3ui32)fwrite(poseGroup->channel, sizeof(a3_SpatialPoseChannel), nodeCount, fp);
				ret += (a3ui32)fwrite(poseGroup->order, sizeof(a3_SpatialPoseEulerOrder), nodeCount, fp);
				...
				else if (*line != '#')
				{
					...
					if (components[section] < sectionComponents[section])
					{
						...
						case htr_basepose:
							...
							if (n >= 0)
							{
								pose = poseGroup_out->hpose->hpose_base + n;
						...
						case htr_nodepose:
							...
							// store channel data
							pose = poseGroup_out->hpose[nodePoses[n] + 1].hpose_base + n;
							a3spatialPoseSetTranslation(pose, translation[0] * globalScale, translation[1] * globalScale, translation[2] * globalScale);
							a3spatialPoseSetRotation(pose, angles[0] * angleConvert, angles[1] * angleConvert, angles[2] * angleConvert);
