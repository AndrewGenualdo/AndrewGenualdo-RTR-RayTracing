...
void a3rendering_update_applyEffectors(a3_Scene_Rendering* scene,
...
void a3rendering_update_animation_skeletal(
	...
	a3boolean const updateIK)
{
	...
	if (updateIK)
	{
		...
		// run solvers
		a3rendering_update_applyEffectors(scene, activeHS_ik, baseHS, poseGroup);
...
void a3rendering_update_animation_other(
	a3_Scene_Rendering* scene, a3f64 const dt)
{
	a3ui32 sampleIndex0;
	a3f64 keyframeParam;
...
void a3rendering_update_animation(
	a3_Scene_Rendering* scene, a3f64 const dt,
	a3boolean const updateIK)
{
	a3rendering_update_animation_skeletal(scene, dt, updateIK);

	a3rendering_update_animation_other(scene, dt);
}
...
void a3rendering_update(a3_DemoState* demoState, a3_Scene_Rendering* scene, a3f64 const dt)
{
	...
	{
		a3rendering_update_animation(scene, dt, updateIK);
	}
	...
	// testing: reset IK effectors to lock them to FK result
	{
		...
		//a3rendering_load_resetEffectors(scene,
...
/*	switch (scene->ctrl_target)
	...
	case rendering_ctrl_character:
		...
		case rendering_input_euler:
			...
			break;
		case rendering_input_kinematic:
			...
			break;
		case rendering_input_interpolate1:
			scene->acc = a3vec2_zero;
			a3real2Set(scene->vel.v,
				a3real_four * (a3real)scene->axis_l[0],
				a3real_four* (a3real)scene->axis_l[1]);
			...
			break;
		case rendering_input_interpolate2:
			a3real2Set(scene->acc.v,
				a3real_four * (a3real)scene->axis_l[0],
				a3real_four * (a3real)scene->axis_l[1]);
